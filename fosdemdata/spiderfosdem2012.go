...
func ExampleScrape(anurl string) {
	...
	*/
	...
	doc.Find(".section").Each(func(i int, s *goquery.Selection) {
		s.Find("#infobox tr").Each(func(i int, infoboxth *goquery.Selection) {
			...
			for i := range infoboxth.Nodes {
				...
				if speaker == true && speakercount > 0 {
					...
					//fmt.Printf("PersonAnode: %s\n", anode)
					//for j := range anode.Attr {
					//	fmt.Printf("Attr: %s\n", anode.Attr[j])
					//	attr := anode.Attr[j]
		...
		linknodes := s.Find(".event-media-link").Nodes
		//fmt.Printf("videonodes: %s\n", videonodes)
		for i := range linknodes {
			linknode := linknodes[i]
			//fmt.Printf("video type: %s\n", reflect.TypeOf(linknode))
			//fmt.Printf("data: %s\n", linknode.FirstChild.Data)
			//for c := linknode.FirstChild; c != nil; c = c.NextSibling {
			//	fmt.Printf("node: %s\n", c.Data)
			//}
			for j := range linknode.Attr {
				//	fmt.Printf("Attr: %s\n", linknode.Attr[j])
				attr := linknode.Attr[j]
				// first we get an href then we get a class.
				// so we first fill in the href and in the next
				// loop we fill in the type of info
				if attr.Key == "href" {
					//fmt.Printf("href: %s", attr.Val)
					alink.Href = attr.Val
					alink.LinkName = linknode.FirstChild.Data
		...
		videonodes := s.Find(".links ul>li>a").Nodes
		//fmt.Printf("videonodes: %s\n", videonodes)
		...
		var title string
		titlenodes := s.Find(".title").Nodes
		for i := range titlenodes {
			titlenode := titlenodes[i]
			//for c := titlenode.FirstChild; c != nil; c = c.NextSibling {
			//fmt.Printf("node: %s\n", c.Data)
			//}
			title = titlenode.LastChild.Data
		}
...
func main() {
	...
	//ExampleScrape("https://archive.fosdem.org/2011/schedule/event/django_architecture.html")
